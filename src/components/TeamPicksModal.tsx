...
export default function TeamPicksModal({
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 sm:p-4 overflow-y-auto bg-black/75 backdrop-blur-xl animate-in fade-in duration-200">
            ...
            >
                {/* Modal Header */}
                <div className="p-4 sm:p-6 border-b border-white/5 bg-gradient-to-r from-emerald-950/40 via-[#0e141a] to-emerald-950/20 relative">
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 sm:top-6 sm:right-6 w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-all cursor-pointer active:scale-90"
                    >
                        <X className="w-4 h-4" />
                    </button>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pr-10">
                        <div className="flex items-center gap-3 min-w-0">
                            <UserAvatar name={managerName} size="lg" />
                            <div className="min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <h3 className="text-lg sm:text-xl font-black text-white truncate">{managerName}</h3>
                                    {isViewingMyself && (
                                ...
                                <p className="text-xs sm:text-sm font-semibold text-emerald-400 truncate mt-0.5">
                                    {teamName}
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Gameweek Navigator */}
                        <div className="flex items-center gap-1.5 bg-black/40 border border-white/10 rounded-xl p-1 self-start sm:self-auto">
                            <button
                    ...
                    {/* Stats & Chip Ribbon */}
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap mt-4 pt-3 border-t border-white/5 text-xs">
                        <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 font-bold">
                            <Trophy className="w-3.5 h-3.5 text-emerald-400" />
                            <span>GW Points:</span>
                            <span className="text-sm font-black text-white tabular-nums">{calculatedGwScore}</span>
                        </div>

                        {transferCost > 0 && (
                            <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-rose-500/15 border border-rose-500/30 text-rose-300 font-bold">
                                <span>Hit: -{transferCost} pts</span>
                            </div>
                        )}

                        {transferCost > 0 && (
                            <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-black/40 border border-white/10 text-gray-300 font-bold">
                                <span>Net: {netGwScore} pts</span>
                            </div>
                        )}

                        {activeChip && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 font-black uppercase text-[10px] tracking-wider animate-pulse">
                        ...
                        <span className="text-gray-500 text-[11px] ml-auto font-mono">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                    {loading ? (
                        ...
                        <div className="py-16 px-6 text-center space-y-3">
                            ...
                            <h4 className="text-base font-bold text-white">Lineup Unavailable</h4>
                            <p className="text-xs text-gray-400 max-w-sm mx-auto leading-relaxed">{error}</p>
                        </div>
                    ) : (
                        <>
                            {/* Visual Pitch */}
                            <div className="relative rounded-3xl p-4 sm:p-6 overflow-hidden border border-emerald-500/25 bg-gradient-to-b from-[#0d2a1b] via-[#091f14] to-[#06150d] shadow-2xl">
                                {/* Pitch Markings */}
                                <div className="absolute inset-0 pointer-events-none opacity-20">
                                    {/* Penalty Box Top */}
                                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-20 border-b-2 border-x-2 border-white rounded-b-xl" />
                                    {/* Center Circle */}
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-36 h-36 rounded-full border-2 border-white" />
                                    {/* Halfway Line */}
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                                    {bench.map((p, idx) => {
                                        ...
                                        return (
                                            <div 
                                                key={p.element}
                                                className="p-2.5 rounded-xl border border-white/5 bg-white/[0.02] flex items-center justify-between gap-2"
                                            >
                                                <div className="min-w-0">
                                                    <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-white/5 text-gray-400 border border-white/5 inline-block mb-1">
                                                        {idx === 0 ? 'GKP 12' : `Sub ${idx}`}
                                                    </span>
                                                    <p className="text-xs font-bold text-white truncate">{player?.web_name || `Player ${p.element}`}</p>
                                                    <p className="text-[10px] text-gray-400">{player?.team_short || 'PL'}</p>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <span className={clsx(
                                                        "text-xs font-black px-2 py-1 rounded-lg block",
                                                        activeChip === 'bboost'
                ...
                <div className="p-4 border-t border-white/5 bg-black/40 flex items-center justify-between gap-3">
                    ...
                    {!isViewingMyself && myTeamId && onSelectTeam ? (
                        <button
                            ...
                            className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 text-xs font-bold transition-all cursor-pointer"
                        >
                        ...
                        <span className="text-[11px] text-gray-500">Official Fantasy Premier League data</span>
                    )}
...
function renderPlayerCard(pick: PlayerPick, player: PlayerDetails | undefined, activeChip: string | null) {
    ...
    return (
        <div 
            key={pick.element}
            className="flex flex-col items-center min-w-[70px] sm:min-w-[84px] max-w-[100px] text-center group"
        >
            {/* Jersey Card */}
            <div className="relative">
                <div className={clsx(
                    "w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex flex-col items-center justify-center p-1 border shadow-lg transition-transform group-hover:scale-105",
                    isCaptain
                        ? "bg-gradient-to-br from-amber-500/30 to-amber-600/10 border-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.3)]"
                        : isVice
                            ? "bg-gradient-to-br from-slate-400/25 to-slate-600/10 border-slate-300 shadow-[0_0_10px_rgba(203,213,225,0.2)]"
                            : "bg-[#101b15]/90 border-white/15 hover:border-emerald-400/40"
                )}>
                    <span className="text-[9px] font-black text-emerald-400 tracking-wider uppercase leading-none">
                        {player?.team_short || 'PL'}
                    </span>
                    <Shirt className={clsx("w-5 h-5 sm:w-6 sm:h-6 mt-0.5", isCaptain ? "text-amber-300" : isVice ? "text-slate-200" : "text-emerald-100")} />
                </div>

                {/* Captaincy Badge */}
                {isCaptain && (
                    <span className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 text-slate-950 font-black text-[10px] flex items-center justify-center border border-white shadow">
                        {activeChip === '3xc' ? '3x' : 'C'}
                    </span>
                )}
                {isVice && !isCaptain && (
                    <span className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-slate-300 text-slate-950 font-black text-[9px] flex items-center justify-center border border-white shadow">
                        VC
                    </span>
                )}
            </div>
            ...
            {/* Name Banner */}
            <div className="w-full mt-1.5 px-1 py-0.5 rounded-md bg-black/70 backdrop-blur-sm border border-white/10">
                ...
                <div className="flex items-center justify-center gap-1 mt-0.5">
                    ...
                    <span className={clsx(
                        "text-[10px] sm:text-[11px] font-black tabular-nums",
